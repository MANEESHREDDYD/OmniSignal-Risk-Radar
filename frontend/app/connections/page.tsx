...
interface Account {id:string;platform:string;account_label:string;account_identifier:string;connection_status:string;last_sync_at:string;messages_synced:number}
...
export default function ConnectionsPage() {
  ...
  return <>
    <PageHeader eyebrow="Connector registry" title="Six channels. Zero credentials." description="Synthetic connectors model the account boundaries and normalization contract required for future OAuth integrations."
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {accounts.map(account=>{const Icon=iconFor(account.platform);return <div key={account.id} className="panel p-5">
    ...
    <div className="mt-5 rounded-2xl border border-dashed border-[#cfcac0] p-5"><div className="text-sm font-semibold">Future connector contract</div><p className="muted mt-1">Gmail API, Microsoft Graph, Slack, Teams, and a local iMessage bridge can plug into this registry later. The normalized message model and scoring engine remain unchanged.</p></div>
  </>;
}
